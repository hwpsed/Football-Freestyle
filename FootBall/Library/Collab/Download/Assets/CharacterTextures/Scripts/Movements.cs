...
public class Movements : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (trans.localPosition.y < minY && !Anim.GetBool("isTackle") && !Anim.GetBool("isTrip"))
        ...
        if (!state.Equals(State.WaitForNextState))
        {
            ...
            {
                updateAnimation();
                if (!Anim.GetBool("isKick") && !Anim.GetBool("isPass") && !Anim.GetBool("isTackle"))
                    setState(State.Move);
            }
            else
                setState(State.Idle);
    ...
    protected float getInputDegree(float ver, float hor)
    {
        ...
        float result = Vector2.Angle(new Vector2(0, 1), vecA);
    ...
    protected void updateMovement(float horizontal, float vertical)
    {
        ...
        float inputDegree = getInputDegree(vertical, horizontal);
    ...
    protected void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.Equals(ball) && !state.Equals(State.Kick) && !state.Equals(State.Pass))
        {
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        if (collision.gameObject.tag.Equals("Footballer"))
        {
            ...
            Debug.Log("1" + conditionList[0].ToString());
            Debug.Log("2" + conditionList[1].ToString());
            Debug.Log("3" + conditionList[2].ToString());
    ...
    public void handleState()
    {
        switch (state)
        {
            ...
            case State.Pass:
                ...
                Anim.Play("Pass", 0, 0.25f);
                setState(State.WaitForNextState);
                break;

            case State.Kick:
                setAnimation("isKick");
                Anim.Play("Kick", 0, 0.25f);
                setState(State.WaitForNextState);
                break;

            case State.Trip:
                Anim.SetBool("isTrip", true);
                Anim.Play("Trip", 0, 0.25f);
                setState(State.WaitForNextState);
                break;

            case State.StandUp:
                Anim.SetBool("isStandUp", true);
                Anim.Play("StandUp", 0, 0.25f);
                setState(State.WaitForNextState);
                break;

            case State.Tackle:
                setAnimation("isTackle", true);
                Anim.Play("Tackle", 0, 0.25f);
